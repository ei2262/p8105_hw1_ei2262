...
#### Why?
A logical vector has two possible levels, true or false. Therefore, when `as.numeric` is applied, the observations in a logical vector is converted to a 0 (false) or to a 1 (true). 0 and 1 are both numeric values and can be used to calculate a mean value. In the logical vector I created in `hw_1df`, 
...
Analyzing the results of `mean()` after applying `as.numeric` helped me understand the why I initially got **NA** as an output. 
